...
* `TEKTON_GIT_USERNAME` – defaults to `RKE2_CLUSTER_GITHUB_USERNAME` or `x-access-token`.
* `TEKTON_GIT_PASSWORD` – defaults to `RKE2_CLUSTER_GITHUB_TOKEN`.
* `TEKTON_GIT_URL` – defaults to `https://${RKE2_CLUSTER_GITHUB_HOST:-github.com}`.
* `TEKTON_DOCKER_CONFIG_JSON` – defaults to `RKE2_CLUSTER_DOCKER_CONFIG_JSON` (base64 JSON for `config.json`).
* `TEKTON_DOCKER_REGISTRY_URL` – defaults to `https://index.docker.io/v1/` unless `RKE2_CLUSTER_DOCKER_REGISTRY_URL` overrides it.
